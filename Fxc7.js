...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans2 = " ~ traduzido por nokia"
			const isUrl = (url) => {
				...
				case 'sticker':
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('nokia', '9672-4995')} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									await costum(fs.readFileSync(ran), sticker, FarhanGans, `~ Aqui está a figurinha animada`)
									fs.unlinkSync(media)
									fs.unlinkSync(ran)
								})
							})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('nokia', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
...
//akhir kreator
		    ...
		    try {
				...
				if (!isOwner) return reply(mess.only.ownerB)
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Marque o membro que você deseja clonar')
